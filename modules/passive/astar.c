...
int astar(int *map, int map_w, int map_h, int startcol, int startrow, int goalcol, int goalrow, int (distance)(int, int, int, int))
{
	...
	pq_modify_or_insert(startrow, startcol, distance(startrow, startcol, goalrow, goalcol), number_of_walls, 0);
	while (!pq_is_empty())
	{
		int r;
		int c;
		...
		if (r == goalrow && c == goalcol)
		{
			...
			for (int i = 0; i < 4; ++i)
			{
				if (map[(r+delta[i][0])*map_w + c+delta[i][1]] == 0)
				{
					...
					if (pq_modify_or_insert(r+delta[i][0], c+delta[i][1], 
                                                                10 * distance(r+delta[i][0], c+delta[i][1], goalrow, goalcol) + root_dist + number_of_walls, 
                                                                root_dist + 1, arrived_from[r+delta[i][0]][c+delta[i][1]][0] + 1))
					{
